...
jsCom = \case
  ...
  DL_ArrayMap _ ans xs as i (DLBlock _ _ f r) -> do
    ...
    f' <- jsPLTail f
    r' <- jsArg r
    return $ "const" <+> ans' <+> "=" <+> "await" <+> jsApply "stdlib.Array_asyncMap" (xs' ++ [(jsApply "async" (as' ++ [i']) <+> "=>" <+> jsBraces (f' <> hardline <> jsReturn r'))])
  DL_ArrayReduce _ ans xs z b as i (DLBlock _ _ f r) -> do
    ...
    f' <- jsPLTail f
    r' <- jsArg r
    return $ "const" <+> ans' <+> "=" <+> "await" <+> jsApply "stdlib.Array_asyncReduce" (xs' ++ [z', (jsApply "async" $ [b'] ++ as' ++ [i']) <+> "=>" <+> jsBraces (f' <> hardline <> jsReturn r')])
  DL_MapReduce {} ->
